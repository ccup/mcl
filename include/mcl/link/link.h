...
#define MCL_LINK_NODE_PREV(ep, link)  (ep)->link.prev

#define MCL_LINK_NODE_INIT(ep, link) do {           	\
    MCL_LINK_NODE_NEXT((ep), link) = (ep);              \
...
#define MCL_LINK_SPLICE_BEFORE(le, e1, en, link) do {   \
    MCL_LINK_NODE_NEXT((en), link) = (le);              \
    MCL_LINK_NODE_PREV((e1), link) = MCL_LINK_NODE_PREV((le), link);  \
    MCL_LINK_NODE_NEXT(MCL_LINK_NODE_PREV((le), link), link) = (e1);  \
    MCL_LINK_NODE_PREV((le), link) = (en);              \
} while (0)

#define MCL_LINK_SPLICE_AFTER(le, e1, en, link) do {    \
    MCL_LINK_NODE_PREV((e1), link) = (le);              \
    MCL_LINK_NODE_NEXT((en), link) = MCL_LINK_NODE_NEXT((le), link);  \
    MCL_LINK_NODE_PREV(MCL_LINK_NODE_NEXT((le), link), link) = (en);  \
    MCL_LINK_NODE_NEXT((le), link) = (e1);              \
} while (0)
...
#define MCL_LINK_UNSPLICE(e1, en, link) do {            \
    MCL_LINK_NODE_NEXT(MCL_LINK_NODE_PREV((e1), link), link) = MCL_LINK_NODE_NEXT((en), link);\
    MCL_LINK_NODE_PREV(MCL_LINK_NODE_NEXT((en), link), link) = MCL_LINK_NODE_PREV((e1), link);\
} while (0)
...
#define MCL_LINK_FOREACH(head, NodeType, link, ep)      \
    for (ep  = MCL_LINK_FIRST(head);                    \
         ...
         ep  = MCL_LINK_NODE_NEXT(ep, link))

#define MCL_LINK_FOREACH_SAFE(head, NodeType, link, e1, ep2) \
    for (e1  = MCL_LINK_FIRST(head), ep2 = MCL_LINK_NODE_NEXT(e1, link); \
         e1 != MCL_LINK_SENTINEL(head, NodeType, link); \
         e1 = ep2, ep2 = MCL_LINK_NODE_NEXT(e1, link))
