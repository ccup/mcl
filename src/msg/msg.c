...
MclStatus MclMsg_Init(MclMsg *self, MsgId id, uint16_t bodySize, void* body) {
...
MclMsg* MclMsg_Create(MsgId id, uint16_t bodySize) {
    MCL_ASSERT_TRUE_NIL(bodySize > 0);

    MclMsg *self = MCL_MALLOC(MclMsg_HeaderSize() + MclAlign_GetSizeOf(bodySize));
    MCL_ASSERT_VALID_PTR_NIL(self);

    if (MCL_FAILED(MclMsg_Init(self, id, bodySize, (uint8_t*)self + MclMsg_HeaderSize()))) {
