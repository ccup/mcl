...
FIXTURE(BridgeTest) {
	TEST("should fill by blue color") {
		...
		FillInfo info = color_fill(&blue.INTF(Color), 12);
	...
	TEST("should fill by red color") {
		ColorRed red;
		color_red_init(&red);
		FillInfo info = color_fill(&red.INTF(Color), 10);
	...
	TEST("should draw red rectangle") {
		...
		shape_rectangle_init(&shape, &red.INTF(Color), 3, 4);
		ShapeInfo info = shape_draw(&shape.INTF(Shape));
	...
	TEST("should draw blue circle") {
		...
		color_blue_init(&blue);
		ShapeCircle shape;
		shape_circle_init(&shape, &blue.INTF(Color), 2);
		ShapeInfo info = shape_draw(&shape.INTF(Shape));
