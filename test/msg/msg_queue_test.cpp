...
FIXTURE(MsgQueueTest) {
    ...
    TEST("should send and recv msg") {
        MCL_AUTO_MSG MclMsg *msg = MclMsg_Create(1, sizeof(uint64_t));
        ...
        MCL_AUTO_MSG MclMsg *result = MclMsg_Create(0, sizeof(uint64_t));
    ...
    TEST("should not send to full mq") {
        uint64_t value = 0xdeadc0de;
        MclMsg msg = MCL_MSG(1, sizeof(value), &value);
        ...
        uint64_t outValue = 0;
        MclMsg result = MCL_MSG(0, sizeof(outValue), &outValue);
        ASSERT_EQ(MCL_SUCCESS, MclMsgQueue_Recv(mq, &result));
    ...
    TEST("should recv from full mq to empty") {
        ...
        MCL_AUTO_MSG MclMsg *msg = MclMsg_Create(0, 2);
        ...
        uint16_t outValue = 0;
        MclMsg result = MCL_MSG(0, sizeof(value), &outValue);
        for (uint16_t i = 0; i < MSG_QUEUE_CAPACITY; i++) {
